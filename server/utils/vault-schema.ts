...
async function ensureBaseSchema(db: ReturnType<typeof createClient>) {
  await db.batch([
    `CREATE TABLE IF NOT EXISTS users (id TEXT PRIMARY KEY, name TEXT NOT NULL, username TEXT NOT NULL UNIQUE, email TEXT NOT NULL UNIQUE, email_verified INTEGER NOT NULL DEFAULT 0, password TEXT NOT NULL, password_hint TEXT, session_version INTEGER NOT NULL DEFAULT 0, created_at TEXT DEFAULT (datetime('now')))`,
    ...
    `CREATE TABLE IF NOT EXISTS email_verification_tokens (id TEXT PRIMARY KEY, user_id TEXT NOT NULL, token_hash TEXT NOT NULL UNIQUE, email TEXT NOT NULL, created_at TEXT NOT NULL DEFAULT (datetime('now')), expires_at TEXT NOT NULL, consumed_at TEXT, FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE)`,
...
async function runVaultSchemaMigration(db: ReturnType<typeof createClient>) {
  ...
  // Email verification was introduced after the first accounts. Accounts that
  // still carry the internal placeholder address never had a real email, so
  // treat them as verified to avoid locking their owners out.
  if (userInfo.rows.length > 0 && !userInfo.rows.some(row => String((row as any).name) === 'email_verified')) {
    await db.execute({ sql: 'ALTER TABLE users ADD COLUMN email_verified INTEGER NOT NULL DEFAULT 0' }).catch((error) => {
      if (!String((error as any)?.message || error).toLowerCase().includes('duplicate column')) throw error
    })
    await db.execute({ sql: "UPDATE users SET email_verified = 1 WHERE email LIKE '%@qvault.invalid' OR email LIKE '%@bitlock.invalid'" })
  }
