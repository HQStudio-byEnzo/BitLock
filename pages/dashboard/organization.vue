<template>
  <div class="section-shell max-w-7xl py-10 md:py-16 space-y-6">
    <section class="hero-panel">
      ...
      <h1 class="mt-3 text-3xl md:text-4xl font-semibold text-foreground">{{ t('organization.title') }}</h1>
    ...
    <div class="grid gap-4 lg:grid-cols-3">
      <section class="glass-panel p-5 space-y-4">
        <div class="flex items-center gap-2"><Icon name="lucide:vault" class="text-accent-600" /><h2 class="text-foreground font-medium">{{ t('organization.vaults') }}</h2></div>
        <form class="flex gap-2" @submit.prevent="create('vault', vaultName)"><input v-model="vaultName" class="input-field" :aria-label="t('organization.vaultName')" :placeholder="t('organization.vaultName')" /><button class="icon-button" :aria-label="t('organization.add')"><Icon name="lucide:plus" /></button></form>
        <ul class="space-y-2"><li v-for="vault in data.vaults" :key="vault.id" class="flex items-center gap-3 border border-border p-3"><span class="h-2 w-2 rounded-full" :style="{ background: vault.color }"/><span class="flex-1 text-sm text-surface-200">{{ vault.name }}</span><span v-if="vault.is_default" class="tech-status">default</span><button v-else class="icon-button" :aria-label="`${t('vault.deleteAction')} ${vault.name}`" @click="remove('vault', vault.id)"><Icon name="lucide:trash-2" class="w-4 h-4" /></button></li></ul>
      </section>

      <section class="glass-panel p-5 space-y-4">
        <div class="flex items-center gap-2"><Icon name="lucide:folder-tree" class="text-accent-600" /><h2 class="text-foreground font-medium">{{ t('organization.folders') }}</h2></div>
        ...
        <ul class="space-y-2"><li v-for="folder in data.folders" :key="folder.id" class="flex items-center gap-3 border border-border p-3"><Icon name="lucide:folder" class="text-surface-500" /><span class="flex-1 text-sm text-surface-200">{{ folder.name }}</span><button class="icon-button" :aria-label="`${t('vault.deleteAction')} ${folder.name}`" @click="remove('folder', folder.id)"><Icon name="lucide:trash-2" class="w-4 h-4" /></button></li></ul>
      </section>

      <section class="glass-panel p-5 space-y-4">
        <div class="flex items-center gap-2"><Icon name="lucide:tags" class="text-accent-600" /><h2 class="text-foreground font-medium">{{ t('organization.tags') }}</h2></div>
        <form class="flex gap-2" @submit.prevent="create('tag', tagName)"><input v-model="tagName" class="input-field" :aria-label="t('organization.tagName')" :placeholder="t('organization.tagName')" /><input v-model="tagColor" type="color" class="h-11 w-12 bg-transparent" :aria-label="t('organization.tagColor')" /><button class="icon-button" :aria-label="t('organization.add')"><Icon name="lucide:plus" /></button></form>
        <ul class="flex flex-wrap gap-2"><li v-for="tag in data.tags" :key="tag.id" class="inline-flex items-center gap-2 border border-border px-3 py-2 text-sm text-surface-200"><span class="h-2 w-2 rounded-full" :style="{ background: tag.color }" />{{ tag.name }}<button class="p-1.5 rounded hover:text-red-600" :aria-label="`${t('vault.deleteAction')} ${tag.name}`" @click="remove('tag', tag.id)"><Icon name="lucide:x" class="w-3.5 h-3.5" /></button></li></ul>
      </section>
    </div>
