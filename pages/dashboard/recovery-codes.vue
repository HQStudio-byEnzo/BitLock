    ...
    <section class="hero-panel space-y-5">
      <div class="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
        ...
        <button @click="showAddModal = true" class="btn-primary flex items-center gap-2 self-start">
          <Icon name="hugeicons:plus" class="w-4 h-4" />{{ t('recovery.addImport') }}
        </button>
      </div>
      <div class="glass-panel p-4 border-accent-500/20 bg-accent-500/5 text-sm text-surface-300">{{ t('recovery.txtNotice') }}</div>
    </section>
    <div v-if="loading" class="flex items-center justify-center py-12"><Icon name="hugeicons:loader" class="w-6 h-6 text-accent-text animate-spin" /></div>
    <div v-else-if="filteredItems.length === 0" class="text-center py-16"><Icon name="hugeicons:ticket-check" class="w-12 h-12 text-surface-600 mx-auto mb-4" /><p class="text-surface-400">{{ t('recovery.empty') }}</p></div>
